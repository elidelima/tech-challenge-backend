...
describe('lib', () => describe('movie', () => {
  ...
  describe('update', () => {
    ...
    it('updates one row from table `movie`, by `id`', async ({context}: Flags) => {
      ...
      const anyReleasedAt = new Date('2000-04-01')
      const anyRuntime = 90
      const anyGenreId = 1
      context.stub.knex_update.resolves()

      await update(anyId, anyName, anyReleasedAt, anyRuntime, anyGenreId)
      sinon.assert.calledOnceWithExactly(context.stub.knex_from, 'movie')
      sinon.assert.calledOnceWithExactly(context.stub.knex_where, { id: anyId })
      sinon.assert.calledOnceWithExactly(context.stub.knex_update, {
        name: anyName, releasedAt: anyReleasedAt, runtime: anyRuntime, genreId: anyGenreId, synopsis: undefined
      })
    })
    ...
    ; [0, 1].forEach( rows =>
      it(`returns ${!!rows} when (${rows}) row is found and deleted`, async ({context}: Flags) => {
        ...
        const anyRuntime = 90
        const anyGenreId = 1
        const anySynopsis = 'test synopsys'
  ...
  describe('create', () => {
    ...
    it('insert one row into table `movie`', async ({context}: Flags) => {
      ...
      sinon.assert.calledOnceWithExactly(context.stub.knex_insert, {
        name: anyName, releasedAt: anyReleasedAt, runtime: anyRuntime, genreId: anyGenreId, synopsis: anySynopsis
      })
    })
